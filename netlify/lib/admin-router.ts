...
export const adminRouter = createRouter({
  ...
  /* ─── Orders (defensive — order_items may not exist) ─── */
  listOrders: adminQuery
    ...
    .query(async ({ input }) => {
      ...
      // Fetch order_items separately (more compatible than embedded query)
      ...
      // Try to fetch items if table exists
      if (orders.length > 0) {
        try {
  ...
  /* ─── Stats (defensive — each query wrapped) ─── */
  getStats: adminQuery.query(async () => {
    ...
    try {
      ...
      console.error("[getStats] product count failed:", e.message);
    }
    ...
    try {
      ...
      console.error("[getStats] order count failed:", e.message);
    }
    ...
    try {
      ...
      console.error("[getStats] sales calc failed:", e.message);
    }
    ...
    try {
      ...
      console.error("[getStats] customer count failed:", e.message);
      // Fallback: count from auth.users via service role
      try {
    ...
    try {
      ...
      console.error("[getStats] latest orders failed:", e.message);
    }
    ...
    try {
      ...
      console.error("[getStats] latest products failed:", e.message);
    }
    ...
    try {
      ...
      console.error("[getStats] activity logs failed:", e.message);
    }
