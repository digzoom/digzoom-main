...
export const adminRouter = createRouter({
  ...
  listProducts: adminQuery
    ...
    .query(async ({ input }) => {
      let query = admin()
        .from("products")
        .select("id,title,price,original_price,discount_percent,is_on_sale,image_url,in_stock,is_active,category_id,created_at,updated_at,description,added_by,updated_by")
  ...
  createProduct: adminQuery
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const imageUrl = input.image_url && input.image_url.trim() !== ""
        ? input.image_url
        : `https://placehold.co/400x400/1a1a2e/FFF?text=${encodeURIComponent(input.title.substring(0, 10))}`;
  ...
  updateProduct: adminQuery
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const { data: oldProduct } = await admin().from("products").select("*").eq("id", id).single();
  ...
  deleteProduct: adminQuery
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const { data: oldProduct } = await admin().from("products").select("*").eq("id", input.id).single();
  ...
  listOrders: adminQuery
    ...
    .query(async ({ input }) => {
      ...
      return (data ?? []).map((o: any) => ({
        ...
        customer_email: o.customer_email, total: o.total, status: o.status,
  ...
  uploadImage: adminQuery
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const { error: uploadError } = await supabase.storage
        .from("product-images")
        .upload(input.filename, buffer, { contentType: input.contentType, upsert: true });

      if (uploadError) throw new Error("Upload failed: " + uploadError.message);

      const { data: urlData } = supabase.storage.from("product-images").getPublicUrl(input.filename);
  ...
  getStats: adminQuery
    .query(async () => {
      ...
      const { data: latestOrders } = await s.from("orders").select("id,order_number,customer_name,total,status,created_at").order("id", { ascending: false }).limit(5);
      const { data: latestProducts } = await s.from("products").select("id,title,price,created_at").order("created_at", { ascending: false }).limit(5);
      const { data: latestActivity } = await s.from("admin_activity_logs").select("*").order("created_at", { ascending: false }).limit(10);
      return {
        productCount: productCount ?? 0, orderCount: orderCount ?? 0,
        totalSales, customerCount: customerCount ?? 0,
        latestOrders: latestOrders ?? [], latestProducts: latestProducts ?? [],
