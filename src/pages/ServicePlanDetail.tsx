...
export default function ServicePlanDetail() {
  ...
  return (
    <main className="min-h-screen bg-[#f5f7fb] pb-24 pt-28 text-slate-950">
      <section className="bg-[#07101e] py-16 text-white">
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-[1.15fr_.85fr] lg:px-8">
          <div>
            ...
            <div className="mt-7 inline-flex rounded-2xl bg-blue-500/15 px-5 py-3 font-black text-blue-200">
              {ar ? plan.productsAr : plan.productsEn}
            </div>
          </div>
          <div className="rounded-[2rem] border border-white/10 bg-white/[.05] p-8">
            <p className="text-sm text-slate-400">
              {ar ? "الاشتراك الشهري" : "Monthly subscription"}
            </p>
            <div className="mt-2 flex items-end gap-2">
              ...
              <span className="pb-1 text-slate-400">
                {ar ? "ر.س / شهر" : "SAR / month"}
              </span>
            </div>
            <Link
              to={`/service-checkout/${plan.id}`}
              className="mt-7 flex min-h-14 items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 font-black hover:bg-blue-500"
            >
              {ar ? "المتابعة للطلب والدفع" : "Continue to order and payment"}
              <Arrow className="h-5 w-5" />
            </Link>
            <p className="mt-3 text-center text-xs leading-5 text-slate-500">
              {ar
                ? "لن يتم الخصم قبل مراجعة التفاصيل والموافقة عليها."
                : "You will not be charged before reviewing and accepting the details."}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="rounded-[2rem] border border-blue-100 bg-blue-50 p-7 md:p-10">
          <p className="text-sm font-black text-blue-700">
            {ar ? "النتيجة التي نعمل عليها" : "THE OUTCOME"}
          </p>
          <h2 className="mt-3 text-2xl font-black leading-10 md:text-3xl">
            {ar ? plan.resultAr : plan.resultEn}
          </h2>
          <div className="mt-6 rounded-2xl bg-white/80 p-5">
            <p className="text-sm font-black text-slate-900">
              {ar ? "لمن تناسب هذه الباقة؟" : "WHO IS THIS FOR?"}
            </p>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-2">
          ...
          />
          <Info
            title={ar ? "طريقة التنفيذ والنشر" : "How publishing works"}
          ...
          />
          <Info
            title={ar ? "المطلوب من العميل" : "What we need from you"}
            icon={<Users className="h-6 w-6" />}
            items={client}
            tone="amber"
          />
          <Info
            title={ar ? "التزام DigZoom" : "DigZoom responsibilities"}
        ...
        <div className="mt-6">
          <Info
            title={
              ar
                ? "كيف نحسب المنشورات والقصص والتسليمات؟"
                : "How deliverables are counted"
            }
        ...
        <div className="mt-6 rounded-[2rem] border border-slate-200 bg-white p-7 md:p-9">
          <div className="flex items-center gap-3">
            <CircleAlert className="h-6 w-6 text-rose-500" />
            <h2 className="text-2xl font-black">
              {ar ? "الحدود والاستثناءات" : "Limits and exclusions"}
            </h2>
          </div>
        ...
        <div className="mt-10 rounded-[2rem] bg-slate-950 p-8 text-center text-white md:p-12">
          <h2 className="text-3xl font-black">
            {ar ? "واضح ما ستحصل عليه؟" : "Ready to proceed?"}
          </h2>
          ...
          <Link
            to={`/service-checkout/${plan.id}`}
            className="mt-7 inline-flex min-h-14 items-center gap-2 rounded-xl bg-blue-600 px-8 font-black hover:bg-blue-500"
          >
            {ar ? "ابدأ طلب الباقة" : "Start plan order"}
            <Arrow className="h-5 w-5" />
          </Link>
        </div>
      </section>
