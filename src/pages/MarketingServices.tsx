...
export default function MarketingServices() {
  ...
  const t = ar ? {
    ...
    preview: "نموذج لوحة المتابعة", illustrative: "عرض توضيحي — تظهر بيانات مشروعك الفعلية بعد ربط القياس",
    ...
    solutions: "الحلول", solutionsTitle: "فريق واحد حول هدف تجاري واضح", solutionsText: "كل حل له دور محدد داخل منظومة النمو. نختار فقط ما يحتاجه مشروعك.",
    ...
    faqTag: "أسئلة شائعة", faqTitle: "قبل أن نبدأ", discover: "اكتشف الحل",
  } : {
    ...
    preview: "Dashboard preview", illustrative: "Illustrative preview — your real data appears after measurement is connected",
    ...
    solutions: "SOLUTIONS", solutionsTitle: "One team around a clear business outcome", solutionsText: "Every solution has a specific role in the growth system. We choose only what your business needs.",
    ...
    faqTag: "FAQ", faqTitle: "Before we begin", discover: "Explore solution",
  };
  ...
  const faqs = ar ? [["هل تقدمون باقات ثابتة؟", "نحدد النطاق بحسب الهدف والمرحلة والميزانية. تحصل على عرض واضح بالتسليمات والمدة والتكلفة قبل البدء."], ["هل يلزم تنفيذ جميع الحلول؟", "لا. نختار أقل مجموعة أعمال تستطيع صنع الأثر المطلوب، ثم نتوسع عندما تدعم البيانات ذلك."], ["هل ميزانية الإعلان ضمن أتعاب الإدارة؟", "لا. ميزانية المنصات تُفصل عن أتعاب الإدارة ويظهر الاثنان بوضوح في العرض."], ["متى تظهر النتائج؟", "يعتمد ذلك على نقطة البداية والهدف والقناة. نوضح مؤشرات مبكرة ونتائج مستهدفة واقعية ضمن خطة كل مشروع."]] : [["Do you offer fixed packages?", "Scope depends on the goal, stage, and budget. You receive clear deliverables, timing, and cost before work begins."], ["Do we need every solution?", "No. We choose the smallest set of work able to create the desired impact."], ["Is ad spend included?", "No. Platform spend is separate from management fees, and both are shown clearly."], ["When should we expect results?", "It depends on the starting point, goal, and channel. We define realistic early indicators for each project."]];
  ...
  return <main className="min-h-screen overflow-hidden bg-[#f5f7fb] text-[#0c1220]">
    <section className="relative isolate min-h-[760px] bg-[#060a12] pb-20 pt-28 text-white md:pb-28 md:pt-36">
      ...
      </div><div className="relative mx-auto w-full max-w-xl"><div className="absolute -inset-8 rounded-full bg-blue-500/15 blur-3xl" /><div className="relative rounded-[2rem] border border-white/10 bg-[#0d1422]/90 p-5 shadow-2xl backdrop-blur-xl sm:p-7"><div className="mb-6 flex items-center justify-between"><div><p className="text-xs font-black tracking-[.18em] text-blue-300">DIGZOOM</p><h2 className="mt-1 text-lg font-black">{t.preview}</h2></div><span className="rounded-xl bg-blue-500/15 p-3 text-blue-300"><Gauge className="h-5 w-5" /></span></div><div className="grid grid-cols-2 gap-3">{t.metrics.map((x, i) => <div key={x} className="rounded-2xl border border-white/[.07] bg-white/[.035] p-4"><p className="text-xs text-slate-400">{x}</p><div className="mt-4 flex items-center gap-2"><span className="h-2 flex-1 rounded-full bg-gradient-to-r from-blue-500 to-violet-500" /><span className={`h-2 w-2 rounded-full ${i < 2 ? "bg-emerald-400" : "bg-blue-400"}`} /></div></div>)}</div><div className="mt-4 rounded-2xl border border-white/[.07] bg-white/[.035] p-4"><svg viewBox="0 0 420 105" className="w-full" aria-label={t.preview}><defs><linearGradient id="growth-line"><stop stopColor="#3b82f6"/><stop offset="1" stopColor="#a855f7"/></linearGradient></defs><path d="M4 90 C45 82,61 88,91 68 S145 75,180 52 S230 63,266 39 S326 50,416 11" fill="none" stroke="url(#growth-line)" strokeWidth="5" strokeLinecap="round"/></svg></div><p className="mt-3 text-center text-[11px] leading-5 text-slate-500">{t.illustrative}</p></div></div></div>
    </section>
