...
export default function Contact() {
  ...
  const [form, setForm] = useState({
    name: "",
    email: "",
    subject: requestedProduct
      ? lang === "ar"
        ? `طلب المنتج رقم ${requestedProduct}`
        : `Product request #${requestedProduct}`
      : "",
    message: requestedProduct
      ? lang === "ar"
        ? "أرغب في طلب هذا المنتج ومعرفة طريقة الدفع المتاحة."
        : "I would like to order this product and confirm the available payment method."
      : "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResult("error");
      toast.error(
        lang === "ar" ? "فشل الاتصال بالخادم" : "Failed to connect to server",
      );
    } finally {
      setSending(false);
    }
  ...
  const contactInfo = [
    ...
    },
    {
      ...
      value: "+966 56 988 8456",
      color: "emerald",
      link: "https://wa.me/00966569888456",
    },
    {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-4">
            {contactInfo.map((c, i) => (
              <div
                key={i}
                className="bg-[#151520] rounded-2xl border border-white/[0.04] p-5 flex items-center gap-4 hover:border-white/[0.08] transition-all"
              >
                <div
                  className={`w-12 h-12 rounded-xl bg-${c.color}-500/10 flex items-center justify-center flex-shrink-0`}
                >
                  {c.icon}
                </div>
                <div>
          ...
          <div className="lg:col-span-2">
            {result === "success" ? (
              ...
              <form
                ...
                className="bg-[#151520] rounded-2xl border border-white/[0.04] p-8"
              >
                {result === "error" && (
